...
image-{{ image_name }}:
  glance:
    ...
    - connection_tenant: {{ pillar['images'][image_name].get('tenant', 'admin') }}
    - connection_password: {{ salt['pillar.get']('keystone:tenants:%s:users:%s:password' % (pillar['images'][image_name].get('tenant', 'admin'), pillar['images'][image_name].get('user', 'admin')), 'ADMIN') }}
